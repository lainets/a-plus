...
class ModuleEntry(DifficultyStats, ModuleEntryBase[ExerciseEntry]):
    ...
    NUM_PARAMS = 2
    INVALIDATORS = [
        (Submission, [post_delete, post_save], with_user_ids(model_module_id)),
        (DeadlineRuleDeviation, [post_delete, post_save], with_user_ids(model_module_id)),
        (MaxSubmissionsRuleDeviation, [post_delete, post_save], with_user_ids(model_module_id)),
        (RevealRule, [post_delete, post_save], with_user_ids(model_module_id)),
        (Submission.submitters.through, [m2m_changed], m2m_submission_userprofile(model_module_id)),
    ]
    _true_children_unconfirmed: bool
    _children_unconfirmed: bool
    invalidate_time: Optional[float]
