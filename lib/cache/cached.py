...
class CacheMeta(type):
    ...
    def invalidate(cls, *models: Any) -> None:
        ...
        logger.info("Invalidating cached data for %s[%s]", cls.__name__, cache_key)
    ...
    def invalidate_many(cls, models_iterable: Collection[Tuple[Any,...]]) -> None:
        ...
        logger.info("Invalidating cached data for %s%s", cls.__name__, cache_keys)
        t = (time(), None)
...
class CacheBase(metaclass=CacheMeta):
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        raise AttributeError("%s not found in %r", name, self)
